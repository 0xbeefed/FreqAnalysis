...
def drawFamily(char, data):
    ...
    chart.create_text(150, 150, text=char + ' :\n' + value, fill='#333333', font=('Arial', 26), justify=CENTER)#Affiche la famille de caractère survolé et son pourcentage d'en le texte
...
def createGraph(data):
    ...
    parameterList = []
    columnList = []
    canvas.delete(ALL) #Efface le graphique précédant afin d'en créer un nouveau, ne fais rien si c'est le premier graphique que l'on veut afficher
...
canvas = Canvas(graphLabel, width=width, height=height, bg='#ffffff')#canvas contiendra le futur graphique
