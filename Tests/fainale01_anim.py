...
def drawFamily(char, data):
    ...
    chart.create_text(150, 10, text=char + ' : ' + value, fill='#333333')
    cache = [];
    for i in range(1, len(family)):
        cache.append(family[i])
    items = sorted(cache, key=lambda x: x[1], reverse=True)
    increment = round(6777215/(len(items))) # on divise 16 777 215  #FFFFFF) par le nombre de donnée voulue
    for i in range(len(items)):
        colors.append('#' + str(format(6000000 + increment*i, '08x')[0:6])) # creation d'un hexa a 6 digits pour la couleur
        ...
        if (int((items[i][1]/totalValue) * 100)) < 0.1:
            ...
            chart.create_oval(20, 20, 280, 280, outline='', fill='#4fade3')
            value = '100%'
        else:
            value = str((int((items[i][1]/totalValue) * 100))) + '%'
        chart.create_text(150, 300 + i * 15, text=items[i][0] + ' : ' + value, fill=colors[i])
